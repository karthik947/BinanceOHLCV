...
const download = async () => {
  const {symbol,timeframe,fromTS,toTS,fileName,tfw} = config.getPrameters();
  ...
  const results = [...header,..._.flatten([...await Promise.all(reqA.map(item => wrapKline(item)))]).map(k => [new Date(k[0]).toLocaleString("en-GB"),k[1],k[2],k[3],k[4],k[5],k[7]])];
  //Save to csv file
